const stat_str = document.getElementById('stat_str');
const icon = document.getElementById('icon');
const location_str = document.getElementById('location_str');
const condition_str = document.getElementById('condition_str');
const temp_str = document.getElementById('temp_str');

/* Get weather data for a particular position (lat/long) */
function showPosition(position) {
  location_str.innerHTML = "acquiring the tmprtr...";
  fetch('https://us-central1-tmprtr-205422.cloudfunctions.net/getTmprtr?lat=' + position.coords.latitude + '&long='+ position.coords.longitude).then(response => response.json()).then(body => {
    console.log(body);
    updateDisplay(body.current_observation);
    $('.loading').hide();
  }).catch(err => {
    $('.loading').hide();
  });
...
function updateDisplay(values) {
  if(values.icon) {
    ...
    stat_str.innerHTML = "";
    condition_str.innerHTML = values.observation_location && values.weather || '';
    temp_str.innerHTML = values.feelslike_c + "&deg;C/" + values.feelslike_f + "&deg;F";
    location_str.innerHTML = values.display_location && values.display_location.full || 'Unknown Location';
  }
  else {
